// godot/utilities.odin — Godot built-in utility function bindings.
//
...
_ensure_utility :: proc "contextless" (uf: ^_UtilFunc, name: cstring, hash: i64) {
	...
	gd.string_name_new_with_latin1_chars(
		cast(gd.UninitializedStringNamePtr)&uf.name_data,
		name,
		true,
	)
	uf.func = gd.variant_get_ptr_utility_function(cast(gd.ConstStringNamePtr)&uf.name_data, hash)
}
...
sin :: proc "contextless" (angle_rad: f64) -> f64 {
	...
	a := angle_rad
	return gd.call_utility_function_ptr_ret(_sin.func, f64, &a)
}

cos :: proc "contextless" (angle_rad: f64) -> f64 {
	_ensure_utility(&_cos, cstring("cos"), 2140049587)
	a := angle_rad
	return gd.call_utility_function_ptr_ret(_cos.func, f64, &a)
}

tan :: proc "contextless" (angle_rad: f64) -> f64 {
	_ensure_utility(&_tan, cstring("tan"), 2140049587)
	a := angle_rad
	return gd.call_utility_function_ptr_ret(_tan.func, f64, &a)
}
...
randf :: proc "contextless" () -> f64 {
	...
	return gd.call_utility_function_ptr_ret(_randf.func, f64)
}
...
// StringRepr holds the 8-byte raw Godot String handle returned by
// utility functions. Not directly printable; convert via variant layer.
StringRepr :: distinct [8]u8

// var_to_str converts a Variant to its string representation.
var_to_str :: proc "contextless" (variable: VariantBytes) -> StringRepr {
	_ensure_utility(&_var_to_str, cstring("var_to_str"), 866625479)
	v := variable
	arg: rawptr = &v
	return gd.call_utility_function_ptr_ret(_var_to_str.func, StringRepr, arg)
}
...
instance_from_id :: proc "contextless" (instance_id: i64) -> gd.ObjectPtr {
	_ensure_utility(&_instance_from_id, cstring("instance_from_id"), 1156694636)
	id := instance_id
	return gd.call_utility_function_ptr_ret(_instance_from_id.func, gd.ObjectPtr, &id)
}
