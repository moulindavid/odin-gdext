...
class_facade_compile_smoke :: proc "contextless" (
	...
	tween: gt.Tween,
	texture_button: gt.TextureButton,
	...
	_, _ = gt.node_get_node_as_range(node, &path_from_node)
	_, _ = gt.node_get_node_as_progress_bar(node, &path_from_node)
	...
	children := gt.node_get_children_default(node)
	gt.typed_array_free(&children)
	_ = gt.node_find_child_default(node, &gt.String{})
	found_children := gt.node_find_children_default(node, &gt.String{})
	gt.typed_array_free(&found_children)
	_ = gt.node_is_part_of_edited_scene(node)
	gt.node_add_to_group_default(node, meta_name)
	gt.node_remove_from_group(node, meta_name)
	_ = gt.node_is_in_group(node, meta_name)
	tree_string := gt.node_get_tree_string(node)
	gt.string_free(&tree_string)
	tree_string_pretty := gt.node_get_tree_string_pretty(node)
	gt.string_free(&tree_string_pretty)
	gt.node_set_scene_file_path(node, &gt.String{})
	node_scene_path := gt.node_get_scene_file_path(node)
	gt.string_free(&node_scene_path)
	...
	gt.node_set_scene_instance_load_placeholder(node, false)
	_ = gt.node_get_scene_instance_load_placeholder(node)
	_ = gt.node_is_node_ready(node)
	...
	texture2d := gt.Texture2D(nil)
	_ = gt.texture_button_is_nil(texture_button)
	_ = gt.range_is_nil(range_handle)
	_ = gt.progress_bar_is_nil(progress_bar)
	_ = gt.texture_button_object_ptr(texture_button)
	_ = gt.range_object_ptr(range_handle)
	_ = gt.progress_bar_object_ptr(progress_bar)
	_, _ = gt.object_ptr_try_as_texture_button(gt.texture_button_object_ptr(texture_button))
	_, _ = gt.object_ptr_try_as_range(gt.range_object_ptr(range_handle))
	_, _ = gt.object_ptr_try_as_progress_bar(gt.progress_bar_object_ptr(progress_bar))
	_ = gt.texture_button_as_base_button(texture_button)
	_ = gt.texture_button_as_control(texture_button)
	_ = gt.texture_button_as_canvas_item(texture_button)
	_ = gt.texture_button_as_node(texture_button)
	_ = gt.texture_button_as_object(texture_button)
	_ = gt.range_as_control(range_handle)
	_ = gt.range_as_canvas_item(range_handle)
	_ = gt.range_as_node(range_handle)
	_ = gt.range_as_object(range_handle)
	_ = gt.progress_bar_as_range(progress_bar)
	_ = gt.progress_bar_as_control(progress_bar)
	_ = gt.progress_bar_as_canvas_item(progress_bar)
	_ = gt.progress_bar_as_node(progress_bar)
	_ = gt.progress_bar_as_object(progress_bar)
	_ = gt.object_is_texture_button(object)
	_, _ = gt.object_try_as_texture_button(object)
	_ = gt.object_is_range(object)
	_, _ = gt.object_try_as_range(object)
	_ = gt.object_is_progress_bar(object)
	_, _ = gt.object_try_as_progress_bar(object)
	_ = gt.node_is_texture_button(node)
	_, _ = gt.node_try_as_texture_button(node)
	_ = gt.node_is_range(node)
	_, _ = gt.node_try_as_range(node)
	_ = gt.node_is_progress_bar(node)
	_, _ = gt.node_try_as_progress_bar(node)
	_ = gt.canvas_item_is_texture_button(canvas_item)
	_, _ = gt.canvas_item_try_as_texture_button(canvas_item)
	_ = gt.canvas_item_is_range(canvas_item)
	_, _ = gt.canvas_item_try_as_range(canvas_item)
	_ = gt.canvas_item_is_progress_bar(canvas_item)
	_, _ = gt.canvas_item_try_as_progress_bar(canvas_item)
	_ = gt.control_is_texture_button(control)
	_, _ = gt.control_try_as_texture_button(control)
	_ = gt.control_is_range(control)
	_, _ = gt.control_try_as_range(control)
	_ = gt.control_is_progress_bar(control)
	_, _ = gt.control_try_as_progress_bar(control)
	_ = gt.base_button_is_texture_button(base_button)
	_, _ = gt.base_button_try_as_texture_button(base_button)
	_ = gt.range_is_progress_bar(range_handle)
	_, _ = gt.range_try_as_progress_bar(range_handle)
	gt.texture_button_set_texture_normal(texture_button, texture2d)
	gt.texture_button_set_texture_hover(texture_button, texture2d)
	gt.texture_button_set_texture_pressed(texture_button, texture2d)
	gt.texture_button_set_texture_disabled(texture_button, texture2d)
	gt.texture_button_set_texture_focused(texture_button, texture2d)
	...
	gt.texture_button_set_flip_h(texture_button, true)
	...
	_ = gt.texture_button_get_texture_pressed(texture_button)
	_ = gt.texture_button_get_texture_disabled(texture_button)
	_ = gt.texture_button_get_texture_focused(texture_button)
	...
	_ = gt.texture_button_is_flipped_v(texture_button)
	gt.range_set_min(range_handle, 0)
	gt.range_set_max(range_handle, 100)
	gt.range_set_step(range_handle, 1)
	gt.range_set_page(range_handle, 10)
	gt.range_set_value(range_handle, 42)
	gt.range_set_value_no_signal(range_handle, 43)
	gt.range_set_as_ratio(range_handle, 0.5)
	gt.range_set_use_rounded_values(range_handle, true)
	gt.range_set_exp_ratio(range_handle, false)
	gt.range_set_allow_greater(range_handle, false)
	gt.range_set_allow_lesser(range_handle, false)
	_ = gt.range_get_value(range_handle)
	_ = gt.range_get_min(range_handle)
	_ = gt.range_get_max(range_handle)
	_ = gt.range_get_step(range_handle)
	_ = gt.range_get_page(range_handle)
	_ = gt.range_get_as_ratio(range_handle)
	_ = gt.range_is_using_rounded_values(range_handle)
	_ = gt.range_is_ratio_exp(range_handle)
	_ = gt.range_is_greater_allowed(range_handle)
	_ = gt.range_is_lesser_allowed(range_handle)
	gt.progress_bar_set_fill_mode(progress_bar, i64(gt.ProgressBarFillMode.fill_begin_to_end))
	gt.progress_bar_set_show_percentage(progress_bar, true)
	gt.progress_bar_set_indeterminate(progress_bar, false)
	gt.progress_bar_set_editor_preview_indeterminate(progress_bar, false)
	_ = gt.progress_bar_get_fill_mode(progress_bar)
	_ = gt.progress_bar_is_percentage_shown(progress_bar)
	_ = gt.progress_bar_is_indeterminate(progress_bar)
	_ = gt.progress_bar_is_editor_preview_indeterminate_enabled(progress_bar)
	...
	gt.resource_set_name(resource, &gt.String{})
	...
	_ = gt.resource_get_local_scene(resource)
	gt.resource_set_id_for_path(resource, &gt.String{}, &gt.String{})
	resource_id := gt.resource_get_id_for_path(resource, &gt.String{})
	gt.string_free(&resource_id)
	_ = gt.resource_is_built_in(resource)
	generated_id := gt.resource_generate_scene_unique_id(resource)
	gt.string_free(&generated_id)
	gt.resource_set_scene_unique_id(resource, &gt.String{})
	scene_id := gt.resource_get_scene_unique_id(resource)
	gt.string_free(&scene_id)
	gt.resource_emit_changed(resource)
	_ = gt.ref_counted_get_reference_count(ref_counted)

	_ = gt.node_add_child_checked(node, node)
	_ = gt.node_add_child_or_destroy_checked(node, node)
	...
	recognized := gt.resource_loader_get_recognized_extensions_for_type(
		resource_loader,
		&resource_load_path,
	)
	gt.packed_string_array_free(&recognized)
	gt.resource_loader_set_abort_on_missing_resources(resource_loader, false)
	dependencies := gt.resource_loader_get_dependencies(resource_loader, &resource_load_path)
	gt.packed_string_array_free(&dependencies)
	_ = gt.resource_loader_has_cached(resource_loader, &resource_load_path)
	_ = gt.resource_loader_get_resource_uid(resource_loader, &resource_load_path)
	directory_entries := gt.resource_loader_list_directory(resource_loader, &resource_load_path)
	...
	loaded_scene_resource, loaded_scene, packed_err, packed_ok :=
		gt.resource_loader_load_packed_scene_owned_checked(resource_loader, &resource_load_path)
	_ = loaded_scene
	gt.require_call_ok(&packed_err)
	if packed_ok {
